...
final class Bank_Mellat_Shortcode extends \DediData\Singleton {
	...
	private function order_mail( $order, $settings ) {
		...
		$fields = '
			...
			<p>نام و نام خانوادگي: ' . $order->order_name_surname . '</p>
			<p>آدرس ايميل: ' . $order->order_email . '</p>
			<p>شماره تلفن: ' . $order->order_phone . '</p>
			<p>توضيحات: ' . $order->order_des . '</p>
			<p>تاريخ: ' . $order->order_date . '</p>
			<p>آي پي: ' . $order->order_ip . '</p>
			<p>مبلغ(ريال): ' . $order->order_amount . '</p>
		...
		$message = str_replace( '%mail_address1%', $settings['email_textLink1'], $message );
		$message = str_replace( '%mail_address2%', $settings['email_textLink2'], $message );
		$message = str_replace( '%mail_address3%', $settings['email_textLink3'], $message );
		...
		$to_address = get_option( 'admin_email' ) . ',' . $order->order_email;
